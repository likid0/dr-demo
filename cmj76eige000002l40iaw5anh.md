---
title: "Breaking the Static Key Habit: Modernizing IBM Storage Ceph S3 Security with STS"
datePublished: Mon Dec 15 2025 13:14:52 GMT+0000 (Coordinated Universal Time)
cuid: cmj76eige000002l40iaw5anh
slug: breaking-the-static-key-habit-modernizing-ibm-storage-ceph-s3-security-with-sts
ogImage: https://cdn.hashnode.com/res/hashnode/image/upload/v1765804482532/603f2368-a6f7-47dc-988f-cb97d6eafebb.png
tags: aws, security, ibm, storage, s3, ceph, sds

---

![](https://cdn.hashnode.com/res/hashnode/image/upload/v1765804320558/be4215b8-cbd0-48f0-9f21-5bd3e4fffd6d.png align="center")

## Introduction: The $148 Million Lesson

In late 2016, [Uber](https://www.uber.com/en-CH/newsroom/2016-data-incident/) learned that intruders had accessed a trove of personal data stored in an Amazon S3 bucket. The entry point was painfully mundane: attackers accessed Uber's source code on GitHub using stolen credentials, found an AWS credential, and used it to access Uber’s data. That single, long-lived credential exposed data on roughly 57 million users and 600,000 drivers.
...
If you are running IBM Storage Ceph Object Gateway (RGW), you face the same pattern: S3 credentials in an application configuration file `config.yaml`, embedded in scripts, or stored in CI/CD variables. Each one is a long-lived credential that, once copied, can be used from anywhere the S3 endpoint is reachable.
...
The Uber incident shows the real cost of a leaked static key. A single exposed AWS access key exposed sensitive data to roughly 57 million users and 600,000 drivers, and Uber later agreed to a $148 million multistate settlement related to the incident and its handling.
...
Yes, but with a critical difference. The service account (e.g., `backup-service`) possesses a static Access Key and Secret Key, but this user has zero permissions to access S3 data. It cannot list buckets, read objects, or delete data.
...
Example Trust Policy (who can assume the role) allowing the user backup-service to assume the role:
...
Example Permission Policy (what the role can do), allowing read-only access to the bucket `backups` :
...
In this post, we'll use inline policies (policies embedded directly in the role). There are other canned policy types available in the IAM API, which we'll cover in the future IAM post.
...
This implementation builds on the IAM foundation covered in [Enhancing Ceph Multitenancy with IAM Accounts](https://ceph.io/en/news/blog/2025/enhancing-ceph-multitenancy-with-iam-accounts/). If you're new to Ceph IAM accounts, that post covers account creation, user management, and policy basics. Here, we focus specifically on enabling STS and using roles for temporary credentials.

For additional STS configuration details and advanced scenarios, see Chapter 7 of the [IBM Storage Ceph Solutions Guide](https://www.redbooks.ibm.com/abstracts/redp5715.html).
...
* **Python 3 & Boto3:** For running the automation scripts (`pip install boto3`).
    
* **IBM Storage Ceph 7.1+:** While basic STS works on older versions, the IAM Accounts feature used in this guide requires Ceph Squid (19.2.0) or newer (IBM Storage Ceph 7.1+).
...
```bash
$ radosgw-admin user create   --account-id=RGW89761398048153888   --uid=backup-admin   --display-name="Backup-Team-Admin"   --account-root   --gen-access-key   --gen-secret
```

The `--account-root` The flag is critical: it designates this user as the account's root user, granting full administrative permissions within the account's scope.

> **Ceph documentation**: *"Account owners are encouraged to use this account root user for management only, and create users and roles with fine-grained permissions for specific applications."*

For this tutorial, we'll use the root user for setup tasks to keep things simple. In production, you typically use the root user to provision IAM users with specific permissions, then revoke or restrict the root user's credentials.
...
Configure two AWS CLI profiles - one for each user. Each profile contains the user's credentials and the RGW/STS endpoint URL, so we don’t need to specify the endpoint on each `aws` cli command. See [AWS CLI configuration](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html) for details.
...
1. The application starts with `backup-service` credentials (long-term, minimal permissions)
    
2. Calls `AssumeRole` using those credentials to request the `backup-reader` role
    
3. Receives temporary credentials (access key + secret + session token)
    
4. Uses temporary credentials for all S3 operations
    
5. Temporary credentials expire after 1 hour (or configured duration)
    
6. Application manually checks expiration before each operation and refreshes if needed
...
Run the example `backup_service.py` Script:
...
By default, STS credentials expire after 1 hour (3600 seconds). If your backup job takes 4 hours, the credentials will expire mid-job, causing authentication failures.
...
RGW Configuration. The following ceph config option controls the global maximum:
...
Enhanced BackupService script example with STS token Auto-Refresh, download from github: [https://gist.github.com/likid0/25519b2f46b63de89f7fe0d2dc9ff283](https://gist.github.com/likid0/25519b2f46b63de89f7fe0d2dc9ff283)

**How it works**:

1. `RefreshableCredentials` Wraps your credential fetching logic
    
2. Before each AWS API call, boto3 checks if credentials are expired or expiring soon
    
3. If needed, boto3 automatically calls `_refresh_credentials()` to get fresh credentials
    
4. Your application never sees authentication errors due to expiration
    
5. Each refresh calls `AssumeRole` using the original service account credentials
...
Starting with **IBM Storage Ceph 8.1**, Ceph RGW now includes a `create_date` timestamp for each access key in the user metadata. This addition enables programmatic key age tracking and automated rotation - a critical capability for eliminating static credential risk.
...
1. **Application queries secrets manager** (no hardcoded credentials):
...
2. **When keys rotate** (automated monthly rotation using the `create_date` field time stamp):
...
3. **Application automatically gets new keys**:
    ...
    * **No code changes required** - application doesn't know rotation happened
        
    * No downtime - old key still works during transition
...
We kept the IAM explanation minimal in this post, just enough to implement STS. In the next post, we'll dive into IAM architecture and access control patterns. We'll cover the new IAM Accounts model introduced in Ceph Squid, how it creates proper multi-tenancy, and why the distinction between Account Root and IAM Users matters for security. We'll explore advanced least privilege patterns, trust policy design for cross-account access, and how to test policies before deployment. We'll also examine organizational mandates, such as blocking ACLs entirely and using the new S3Control API for account-level governance.